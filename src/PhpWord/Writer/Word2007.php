...
use Alireza1250ssss\PhpWord\Media;
...
class Word2007 extends AbstractWriter implements WriterInterface
{
     ...
     *
     * @param \Alireza1250ssss\PhpWord\PhpWord
     */
    public function __construct(PhpWord $phpWord = null)
    {
        ...
        foreach (array_keys($this->parts) as $partName) {
            ...
            if (class_exists($partClass)) {
                /** @var \Alireza1250ssss\PhpWord\Writer\Word2007\Part\AbstractPart $part Type hint */
                $part = new $partClass();
     ...
     *
     * @param \Alireza1250ssss\PhpWord\Shared\ZipArchive $zip
        ...
        if (!empty($elements)) {
            foreach ($elements as $file => $media) {
                if (count($media) > 0) {
                    ...
                    /** @var \Alireza1250ssss\PhpWord\Writer\Word2007\Part\AbstractPart $writerPart Type hint */
    ...
    private function addNotes(ZipArchive $zip, &$rId, $noteType = 'footnote')
    {
        ...
        /** @var \Alireza1250ssss\PhpWord\Collection\AbstractCollection $collection Type hint */
    ...
    private function addChart(ZipArchive $zip, &$rId)
    {
        ...
        $index = 0;
        if ($collection->countItems() > 0) {
            /** @var \Alireza1250ssss\PhpWord\Element\Chart $chart */
