...
use Alireza1250ssss\PhpWord\Element\AbstractContainer;
use Alireza1250ssss\PhpWord\Element\Row;
...
class Html
{
     ...
     *
     * @param \Alireza1250ssss\PhpWord\Element\AbstractContainer $element Where the parts need to be added
                ...
                switch (strtolower($attribute->name)) {
                    ...
                    case 'width':
                        ...
                        if (false !== strpos($val, '%')) {
                            ...
                            $styles['unit'] = \Alireza1250ssss\PhpWord\SimpleType\TblWidth::PERCENT;
                        } else {
                            // e.g. <table width="250> where "250" = 250px (always pixels)
                            $styles['width'] = Converter::pixelToTwip($val);
                            $styles['unit'] = \Alireza1250ssss\PhpWord\SimpleType\TblWidth::TWIP;
                        }
                        break;
                    case 'cellspacing':
     ...
     * @param \Alireza1250ssss\PhpWord\Element\AbstractContainer $element object to add an element corresponding with the node
    ...
    protected static function parseNode($node, $element, $styles = array(), $data = array())
    {
        ...
        if (isset($nodes[$node->nodeName])) {
            ...
            $method = "parse{$method}";
            $newElement = call_user_func_array(array('Alireza1250ssss\PhpWord\Shared\Html', $method), array_values($arguments));
     ...
     *
     * @param \DOMNode $node
     * @param \Alireza1250ssss\PhpWord\Element\AbstractContainer $element
     * @param array $styles
     * @param array $data
     */
     ...
     * @param \Alireza1250ssss\PhpWord\Element\Table $element
     * @param array &$styles
     * @return \Alireza1250ssss\PhpWord\Element\Cell|\Alireza1250ssss\PhpWord\Element\TextRun $element
     */
    ...
    protected static function parseList($node, $element, &$styles, &$data)
    {
        ...
        if (isset($data['listdepth'])) {
            ...
            /** @var \Alireza1250ssss\PhpWord\Style\NumberingLevel */
            $level = $levels[0];
            if ($start > 0) {
        ...
        foreach ($properties as $property) {
            ...
            switch ($cKey) {
                ...
                case 'line-height':
                    $matches = array();
                    if ($cValue === 'normal') {
                        $spacingLineRule = \Alireza1250ssss\PhpWord\SimpleType\LineSpacingRule::AUTO;
                        $spacing = 0;
                    } elseif (preg_match('/([0-9]+\.?[0-9]*[a-z]+)/', $cValue, $matches)) {
                        //matches number with a unit, e.g. 12px, 15pt, 20mm, ...
                        $spacingLineRule = \Alireza1250ssss\PhpWord\SimpleType\LineSpacingRule::EXACT;
                ...
                case 'width':
                    if (preg_match('/([0-9]+[a-z]+)/', $cValue, $matches)) {
                        ...
                        $styles['unit'] = \Alireza1250ssss\PhpWord\SimpleType\TblWidth::AUTO;
                    }
                    break;
                case 'border':
                case 'border-top':
     ...
     * @param \Alireza1250ssss\PhpWord\Element\AbstractContainer $element
     *
     * @return \Alireza1250ssss\PhpWord\Element\Image
     **/
        ...
        foreach ($node->attributes as $attribute) {
            switch ($attribute->name) {
                ...
                case 'width':
                    ...
                    $style['width'] = $width;
                    $style['unit'] = \Alireza1250ssss\PhpWord\Style\Image::UNIT_PX;
                    break;
                case 'height':
                    ...
                    foreach ($styleattr as $attr) {
                        if (strpos($attr, ':')) {
                            ...
                            switch ($k) {
                                case 'float':
                                    if (trim($v) == 'right') {
                                        $style['hPos'] = \Alireza1250ssss\PhpWord\Style\Image::POS_RIGHT;
                                        $style['hPosRelTo'] = \Alireza1250ssss\PhpWord\Style\Image::POS_RELTO_MARGIN; // inner section area
                                        $style['pos'] = \Alireza1250ssss\PhpWord\Style\Image::POS_RELATIVE;
                                        $style['wrap'] = \Alireza1250ssss\PhpWord\Style\Image::WRAP_TIGHT;
                                        $style['overlap'] = true;
                                    }
                                    if (trim($v) == 'left') {
                                        $style['hPos'] = \Alireza1250ssss\PhpWord\Style\Image::POS_LEFT;
                                        $style['hPosRelTo'] = \Alireza1250ssss\PhpWord\Style\Image::POS_RELTO_MARGIN; // inner section area
                                        $style['pos'] = \Alireza1250ssss\PhpWord\Style\Image::POS_RELATIVE;
                                        $style['wrap'] = \Alireza1250ssss\PhpWord\Style\Image::WRAP_TIGHT;
                                        $style['overlap'] = true;
    ...
    /**
     * Transforms a HTML/CSS alignment into a \Alireza1250ssss\PhpWord\SimpleType\Jc
     *
